  ...
  return (
    <Modal onClose={props.onClose}>
      ...
      <div className={classes.total}>
        {props.link&&<Link to={`download/${props.link}`}>{`http://localhost:3000/download/${props.link}`}</Link>}
        {props.link&&<Link to={`revoke/${props.link}`}>{`http://localhost:3000/revoke/${props.link}`}</Link>}
